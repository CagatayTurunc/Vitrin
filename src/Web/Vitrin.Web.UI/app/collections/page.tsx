...
import { Bookmark, Globe2, LayoutGrid, Loader2, Lock, Plus, Sparkles, Users } from 'lucide-react'
...
type CollectionTab = 'public' | 'following' | 'mine' | 'shared'
...
export default function CollectionsPage() {
  ...
  const fetchCollections = useCallback(async () => {
    ...
    try {
      ...
      const [response, editorialResponse, followingResponse] = await Promise.all([
        fetch(`${apiUrl}/api/collections`, { headers }),
        fetch(`${apiUrl}/api/collections/editorial`, { headers }),
        accessToken ? fetch(`${apiUrl}/api/collections/following`, { headers }) : Promise.resolve(null),
      ])
      const base = response.ok ? await response.json() as CollectionSummary[] : []
      const editorial = editorialResponse.ok ? await editorialResponse.json() as Array<CollectionSummary & { followerCount: number; isFollowing: boolean }> : []
      const editorialById = new Map(editorial.map(item => [item.id, item]))
      setCollections(base.map(item => ({ ...item, ...(editorialById.get(item.id) ?? {}), isEditorial: editorialById.has(item.id) })))
      setFollowedIds(followingResponse?.ok ? await followingResponse.json() as string[] : [])
    } finally {
      setIsLoading(false)
    }
  }, [accessToken])
  ...
  const visibleCollections = useMemo(() => collections.filter(collection => {
    ...
    if (activeTab === 'following') return followedIds.includes(collection.id)
    return collection.visibility === CollectionVisibility.Public
  }), [activeTab, collections, followedIds])
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto rounded-2xl border border-border bg-card p-1.5">
        {([
          ...
          ['following', 'Takip ettiklerim', Bookmark],
              ...
              <Link key={collection.id} href={`/collection/${collection.slug}`} className="group block">
                <article className="relative flex h-full min-h-56 flex-col overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-lg">
                  ...
                  <div className="mb-5 flex items-center justify-between">
                    ...
                    {collection.isEditorial ? <span className="flex items-center gap-1 text-xs font-bold text-amber-600"><Sparkles className="h-3.5 w-3.5" /> Editör seçkisi</span> : collection.isOwner && <span className="text-xs font-bold text-primary">Sahibi sensin</span>}
                  </div>
                  ...
                  <div className="mt-6 flex items-center justify-between border-t border-border pt-4 text-xs font-semibold text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{collection.followerCount ?? 0} takipçi</span>
                  </div>
                </article>
              </Link>
            )
