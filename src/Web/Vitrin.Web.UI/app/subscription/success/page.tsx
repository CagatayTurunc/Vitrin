...
const TIER_CONFIG: Record<string, { label: string; icon: typeof Crown; gradient: string; badge: string }> = {
  ProMaker: {
    label: 'Pro Maker',
    icon: Crown,
...
export default function SubscriptionSuccessPage() {
  ...
  useEffect(() => {
    setMounted(true)
    // Session'ı yenile — yeni tier bilgisi için
    update()
  }, [update])
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className={`absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[400px] rounded-full blur-3xl opacity-10 bg-gradient-to-br ${config.gradient}`} />
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 text-left space-y-3">
          ...
          {tier === 'ProMaker' ? (
            <ul className="space-y-2">
              {['Sınırsız ürün paylaşımı', 'Günlük 50 AI önerisi', '🏆 PRO rozeti profilde', 'Öncelikli listeleme', '90 gün gelişmiş analitik'].map(f => (
            ...
            <ul className="space-y-2">
              {["Pro'daki her şey", '10 ekip üyesi', '💎 FEATURED rozeti', 'API erişimi', 'Öncelikli destek'].map(f => (
                <li key={f} className="flex items-center gap-2 text-sm">
                  <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" />
                  {f}
                </li>
