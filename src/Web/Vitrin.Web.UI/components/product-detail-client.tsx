...
import { ArrowUp, ExternalLink, MessageSquare, Share2, AlertCircle, Bookmark, X, ChevronLeft, ChevronRight, Maximize2, Star, Check, Copy, Bell } from "lucide-react";
import { LoginModal } from "@/components/login-modal";
import { AddToCollectionModal } from "@/components/add-to-collection-modal";
import { ReportDialog } from "@/components/report-dialog";
import dynamic from "next/dynamic";
import "@uiw/react-markdown-preview/markdown.css";
import type { ProductApiModel, ProductDetailApiModel } from "@/core/domain/product.types";
import type { UserProfile } from "@/core/domain/user.types";
import { ProductRepository } from "@/core/infrastructure/product.repository";
import { ProductCommunityPanel } from "@/components/product-community-panel";
...
export function ProductDetailClient({ slug, initialProduct }: { slug: string; initialProduct: ProductDetailApiModel }) {
  const { data: session } = useSession();
  const accessToken = session?.accessToken;

  const [product, setProduct] = useState<ProductDetailApiModel | null>(initialProduct);
  ...
  const [isFollowingProduct, setIsFollowingProduct] = useState(false);
  const [productFollowerCount, setProductFollowerCount] = useState(0);
  const [isProductFollowLoading, setIsProductFollowLoading] = useState(false);
  const [isVoting, setIsVoting] = useState(false);
  const [isVoteCelebrating, setIsVoteCelebrating] = useState(false);
  const [shareCopied, setShareCopied] = useState(false);
  const [makerProducts, setMakerProducts] = useState<ProductApiModel[]>([]);
  ...
  const getShareDetails = () => ({
    ...
    title: product?.name ?? "Vitrin ürünü",
    text: product ? `${product.name} — ${product.tagline}` : "Vitrin'de keşfettiğim ürüne göz at.",
  });

  const copyShareLink = async () => {
    try {
      await navigator.clipboard.writeText(getShareDetails().url);
      setShareCopied(true);
      window.setTimeout(() => setShareCopied(false), 2000);
    } catch (error) {
      console.warn("Paylaşım bağlantısı kopyalanamadı:", error);
    }
  };
  ...
  const openSocialShare = (network: "x" | "linkedin" | "whatsapp") => {
    const details = getShareDetails();
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (productData.makerId) {
          ...
          try {
            const products = await ProductRepository.getMakerProducts(productData.makerId);
            setMakerProducts(products.filter((item) => item.id !== productData.id).slice(0, 3));
          } catch (error) {
            console.warn("Maker'ın diğer ürünleri şu anda alınamadı:", error);
            setMakerProducts([]);
          }
        }
      }
  ...
  useEffect(() => {
    ...
    let isCurrent = true;
    void fetch(`${API_URL}/api/products/${product.id}/follow`, {
      headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : undefined,
    }).then(async (response) => response.ok ? await response.json() as { isFollowing: boolean; followerCount: number } : null)
      .then((status) => {
        if (!isCurrent || !status) return;
        setIsFollowingProduct(status.isFollowing);
        setProductFollowerCount(status.followerCount);
      })
      .catch((error) => console.warn("Ürün takip durumu alınamadı:", error));
  ...
  const handleFollowProduct = async () => {
    ...
    if (!product || isProductFollowLoading) return;

    const previous = isFollowingProduct;
    setIsProductFollowLoading(true);
    setIsFollowingProduct(!previous);
    setProductFollowerCount((count) => Math.max(0, count + (previous ? -1 : 1)));
    try {
      const response = await fetch(`${API_URL}/api/products/${product.id}/follow`, {
        method: previous ? "DELETE" : "PUT",
        headers: { Authorization: `Bearer ${session.accessToken}` },
      });
      if (!response.ok) throw new Error(`Follow request failed with status ${response.status}`);
      const status = await response.json() as { isFollowing: boolean; followerCount: number };
      setIsFollowingProduct(status.isFollowing);
      setProductFollowerCount(status.followerCount);
    } catch (error) {
      console.error(error);
      ...
      setIsProductFollowLoading(false);
    }
  };
  ...
  const handleUpvote = async () => {
    ...
    try {
      ...
      if (!previousVoteState) {
        ...
        window.setTimeout(() => setIsVoteCelebrating(false), 700);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20">
        ...
        <div className="flex gap-6 items-center">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {product.activeLaunch && <span className="rounded-full bg-orange-500/10 px-2.5 py-1 text-xs font-bold text-orange-700">{product.activeLaunch.versionLabel}</span>}
              {product.categories?.map((category) => (
                <Link key={category.id} href={`/category/${category.slug}`} className="rounded-full bg-violet-500/10 px-2.5 py-1 text-xs font-bold text-violet-700 hover:bg-violet-500/20">
                  {category.name}
                </Link>
              ))}
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
          {product.websiteUrl ? (
            <a
              ...
              className={buttonVariants({ variant: "outline", size: "lg", className: "gap-2 h-14 px-6 rounded-xl font-semibold w-full md:w-auto" })}
            >
              Geliştirici Sitesi <ExternalLink className="h-4 w-4" />
            </a>
          ) : (
            <Button size="lg" variant="outline" disabled className="gap-2 h-14 px-6 rounded-xl font-semibold w-full md:w-auto">
              Site bağlantısı yok <ExternalLink className="h-4 w-4" />
            </Button>
          )}
          <Button
            size="lg"
            variant={isFollowingProduct ? "secondary" : "outline"}
            className="h-14 gap-2 rounded-xl px-5 font-semibold"
            onClick={handleFollowProduct}
            disabled={isProductFollowLoading}
          >
            <Bell className={`h-4 w-4 ${isFollowingProduct ? "fill-current" : ""}`} />
            {isFollowingProduct ? "Takip ediliyor" : "Ürünü takip et"}
            {productFollowerCount > 0 && <span className="text-xs opacity-70">({productFollowerCount})</span>}
          </Button>
          <Button 
            size="lg" 
            className={`relative isolate gap-2 h-14 overflow-visible px-8 rounded-xl font-bold w-full md:w-auto bg-[#007A52] hover:bg-[#006B48] text-white shadow-md transition-all hover:scale-105 active:scale-95 ${isVoteCelebrating ? "ring-4 ring-emerald-400/30" : ""}`}
            onClick={handleUpvote}
            disabled={isVoting}
          >
            {isVoteCelebrating && <span aria-hidden className="pointer-events-none absolute inset-0 -z-10 animate-ping rounded-xl bg-emerald-400/40" />}
            <ArrowUp className={`h-5 w-5 ${isVoteCelebrating ? "animate-bounce" : ""}`} />
            {hasVoted ? "UPVOTED" : "UPVOTE"}
            {product.upvotes > 0 && <span className="opacity-90 ml-1">({product.upvotes})</span>}
          </Button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <Link href={maker?.username ? `/profile/${maker.username}` : "/profile"} className="font-semibold hover:underline">
          ...
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <Button variant="secondary" className="w-full justify-start gap-2 rounded-xl" onClick={() => void shareProduct()}>
                <Share2 className="h-4 w-4" /> Paylaş
              </Button>
              <div className="grid grid-cols-4 gap-2 pt-1" aria-label="Sosyal paylaşım seçenekleri">
                <button type="button" onClick={() => openSocialShare("x")} className="inline-flex h-9 items-center justify-center rounded-lg border bg-background text-xs font-bold transition-colors hover:bg-muted" aria-label="X'te paylaş">
                  X
                </button>
                <button type="button" onClick={() => openSocialShare("linkedin")} className="inline-flex h-9 items-center justify-center rounded-lg border bg-background transition-colors hover:bg-muted" aria-label="LinkedIn'de paylaş">
                  <span className="text-xs font-extrabold text-[#0A66C2]">in</span>
                </button>
                <button type="button" onClick={() => openSocialShare("whatsapp")} className="inline-flex h-9 items-center justify-center rounded-lg border bg-background text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-500/10" aria-label="WhatsApp'ta paylaş">
                  WA
                </button>
                <button type="button" onClick={() => void copyShareLink()} className="inline-flex h-9 items-center justify-center rounded-lg border bg-background transition-colors hover:bg-muted" aria-label="Bağlantıyı kopyala">
      ...
      {makerProducts.length > 0 && (
        <section className="border-t border-border/50 pt-10" aria-labelledby="maker-products-title">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-600">Maker koleksiyonu</p>
              <h3 id="maker-products-title" className="mt-1 text-2xl font-bold">
                Bu maker&apos;ın diğer ürünleri
              </h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {maker?.fullName ?? "Bu maker"} tarafından yayınlanan diğer ürünler.
              </p>
            </div>
            {maker?.username && (
              <Link href={`/profile/${maker.username}`} className="shrink-0 text-sm font-semibold text-emerald-700 hover:underline">
                Profili gör
              </Link>
            )}
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {makerProducts.map((item) => (
              <Link key={item.id} href={`/product/${item.slug}`} className="group flex min-w-0 items-center gap-4 rounded-2xl border bg-card p-4 transition-all hover:-translate-y-0.5 hover:border-emerald-500/30 hover:shadow-md">
                <div className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl border bg-muted">
                  {item.thumbnailUrl ? (
                    <Image src={item.thumbnailUrl} alt={item.name} fill sizes="64px" className="object-cover transition-transform group-hover:scale-105" />
                  ) : (
                    <span className="font-bold text-muted-foreground">{item.name.slice(0, 2).toUpperCase()}</span>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="truncate font-bold transition-colors group-hover:text-emerald-700">{item.name}</h4>
                  ...
                  <span className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-emerald-700">
                    <ArrowUp className="h-3 w-3" /> {item.upvotes ?? 0}
                  </span>
                </div>
              </Link>
            ))}
      ...
      <ProductCommunityPanel productId={product.id} productSlug={product.slug} />
